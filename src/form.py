...
class TypeformSubmitter:
    ...
    def create_new_session(self, proxy: str = None):
    ...
    def _generate_browser_fingerprint(self):
        fingerprint = f"{self.user_agent}{time.time()}{random.randint(1, 1000000)}"
        return hashlib.md5(fingerprint.encode()).hexdigest()

    def start_submission(self, proxy: str = None) -> None:
        self.create_new_session(proxy)
        ...
        headers = {
            ...
            "X-Typeform-Unique-ID": str(uuid.uuid4()),
        ...
        data = {
            ...
            "cookie_enabled": True,
            "do_not_track": random.choice([None, "0", "1"])
        }
...
def submit_typeform(data: Dict[str, Any], proxies: List[str]) -> Dict[str, Any]:
    submitter = TypeformSubmitter(FORM_ID)
    for attempt in range(3):
        try:
            proxy = random.choice(proxies)
            submitter.start_submission(proxy)
            ...
            logger.error(f"Request error during form submission: {str(e)}")
            if attempt < 2:
                logger.info(f"Retrying with a different proxy. Attempt {attempt + 2}/3")
